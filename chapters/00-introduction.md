...
This book covers computational finance with AI in the workflow. The chapter-level *A note about AI* sections address the local quantitative methods; the framing chapter has the most substantive treatment. The unifying principle: **the model is a tutor on technique, not a counselor on decisions.** It can explain Black-Scholes, walk through portfolio optimization, derive duration, and produce code for any standard quantitative method. It cannot replace the working analyst's judgment about which model fits which market regime, which assumption is doing the work, and which number to actually trust. Use the model to learn. Verify every specific number against the source data. The investment decisions belong to licensed professionals.
